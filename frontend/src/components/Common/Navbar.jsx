  ...
  return (
    ...
    <nav className=' container mx-auto flex items-center justify-between py-4 px-6'>
        ...
        <div className=' flex items-center space-x-4'>
            ...
            <Link to='/profile' className=' hover:text-black'> <HiOutlineUser className=' h-5 w-6 text-gray-700'/> </Link>
            <button  onClick={toggleCartDrawer} className=' relative hover:text-black'>
                <HiOutlineShoppingBag className='h-6 w-6 text-gray-700'/>
                {cartItemCount > 0  && (
                    <span className=' absolute -top-1  bg-rabbit-red text-white text-xs rounded-full px-1.5 py-0.5'>{cartItemCount}</span>
                )}
